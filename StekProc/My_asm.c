...
int compile(FILE* source, byte* buffer);
...
int main(int argc, char* argv[]){
	...
	byte* buffer = (byte*) calloc (MAX_BYTE_CODE_SIZE, sizeof(byte));
	assert(buffer);
	...
	if (fwrite(buffer, sizeof(byte), buffer_size, byte_code) != buffer_size) {
		...
		switch (tempcom[0]) {
			case 'p':
				switch (tempcom[1]) {
					...
					else if (sscanf(temparg, "%d", &tempval) == 0) {
						printf("incorrect argument in operator push %d : %s\n", counter\
						, temparg);
						return 0;
					}
